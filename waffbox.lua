...
local function loadSettings()
	...
	if not success then 
		if writefile then
			warn('WaffBox had an issue accessing configuration saving capability.')
		end
	end
end
...
local WaffBox = useStudio and script.Parent:FindFirstChild('WaffBox') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	...
	if not warned then
		warn('WaffBox | Build Mismatch')
		print('WaffBox may encounter issues as you are running an incompatible interface version ('.. ((WaffBox:FindFirstChild('Build') and WaffBox.Build.Value) or 'No Build') ..').\n\nThis version of WaffBox is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, WaffBox = WaffBox, useStudio and script.Parent:FindFirstChild('WaffBox') or game:GetObjects("rbxassetid://10804731440")[1]
...
if gethui then
	WaffBox.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(WaffBox)
	WaffBox.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	WaffBox.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == WaffBox.Name and Interface ~= WaffBox then
			Interface.Enabled = false
			Interface.Name = "WaffBox-Old"
		end
	end
elseif not useStudio then
...
if WaffBox.AbsoluteSize.X < minSize.X and WaffBox.AbsoluteSize.Y < minSize.Y then
...
local MPrompt = WaffBox:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = WaffBox:FindFirstChild('Drag')
...
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/WaffBox/refs/heads/main/icons.lua')
-- Variables
...
local function ChangeTheme(Theme)
	...
	WaffBox.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		WaffBox.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		WaffBox.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(WaffBox:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("WaffBox | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("WaffBox | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
function WaffBoxLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('WaffBox | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('WaffBox Settings', 0, true)

	if TabList['WaffBox Settings'] then
		TabList['WaffBox Settings'].LayoutOrder = 1000
	end

	if Elements['WaffBox Settings'] then
		Elements['WaffBox Settings'].LayoutOrder = 1000
	end
...
function WaffBoxLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "WaffBox"
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					...
					warn('Check docs.sirius.menu for help with WaffBox specific development.')
				end
			end
		end
	...
	if not success then warn('WaffBox had an issue creating settings.') end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['WaffBox Settings'])
		end)
	end)
...
if useStudio then
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using WaffBox may overwrite your key file
